...
int main(int argc, char **argv) {
    ...
    SYN_Packet.seqNum = 1000; //initial sequence number
    SYN_Packet.ackNum = 999999999; //we don't know initial ack number
    SYN_Packet.SYN = 1;
    SYN_Packet.FIN = 0;
    ...
    //====================== 3 WAY HANDSHAKE BEGIN ====================
    n = sendto(sockfd, (struct TCP_Packet *) &SYN_Packet, sizeof(SYN_Packet), 0, (const struct sockaddr *) &serveraddr, serverlen); //==1st SYN sendinging to server
    if (n < 0) {
    ...
    n = recvfrom(sockfd, SYNACK_Packet, sizeof(*SYNACK_Packet), 0, (struct sockaddr *) &serveraddr, &serverlen); //=======2nd SYN_ACK receiving from server
    printf("Receiving packet %d\n", SYNACK_Packet->ackNum);
    
    // Make the Packet sending requested filename
    TCP_Packet Request_Packet;
    Request_Packet.seqNum = SYNACK_Packet->ackNum; //seqnum will be the sequence number, 3rd handshake
    Request_Packet.ackNum = SYNACK_Packet->ackNum; //ACK will be the sequence number, 3rd handshake
    ...
    // Then send it to the server
    n = sendto(sockfd, (struct TCP_Packet *) &Request_Packet, sizeof(Request_Packet), 0, (const struct sockaddr *) &serveraddr, serverlen); //======= 3rd seding ACK to server
    if (n < 0) {
        perror("sendto did not work");
        exit(1);
    }
...
//        TCP_Packet * Result_Packet = malloc(sizeof(TCP_Packet));
        ...
        ackPacket.ackNum = Result_Packet->ackNum;
        
        int finished = Result_Packet->FIN;
