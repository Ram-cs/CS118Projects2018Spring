...
int main(int argc, char **argv) {
    ...
    vector history_buffer;
    vector_init(&history_buffer);
    
    int *get_sequence = malloc(sizeof(vector));
    ...
    while(finish_file == 0)
    {
        ...
        if (r > 0)
        {
            ...
            if (thePoll[0].revents & POLLIN)
            {
                ...
                for(int i = 0; i < history_buffer.total; i++) {
                    get_sequence = vector_get(&history_buffer, i);
                    if(Result_Packet->seqNum == *get_sequence) {
                        
                        ackPacket.seqNum = Result_Packet->seqNum;
