...
int main(int argc, char *argv[])
{
	...
	int fd = open(fileName, O_RDONLY);
        ...
        if(fd <= 0) {
            ...
            write(newsockfd, "\r\n <html> <body> <h1> 404 Not Found </h1> </body> </html>",57);
	    write(newsockfd, "\r\n", 2);
	    ...
	    // Alternative way to get Content-Type Field
	    /*
	    char contentBuffer[25] = "Content-Type: ";
	    int contentPos = 14;
	    for (int k = 0; fileType[k] != '\0'; contentPos++, k++)
	      {
		contentBuffer[contentPos] = fileType[k];
	      }
	    contentBuffer[contentPos] = '\r';
	    contentBuffer[contentPos+1] = '\n';
	    contentBuffer[contentPos+2] = '\0';
	    contentPos += 2;
	    */

	    //write(newsockfd, "HTTP/1.1 200 OK\r\n", 17);
	    //write(newsockfd, contentBuffer, contentPos);
	    //write(newsockfd, "\r\n", 2);
	    ...
	    while ((byteCount = read(fd, fileBuffer, 1)) > 0)
	      write(newsockfd, fileBuffer, 1);
